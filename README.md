<div align="center">

# Hi 👋, I'm Yunus

### Full-Stack Developer · Discord Bot Developer · AI & Automation

I build web applications, Discord bots, backend systems and automation tools.

</div>

---

## 👨‍💻 About Me

- 💻 Full-Stack Developer
- 🤖 Discord Bot Developer
- ⚡ Building backend systems and automation tools
- 🧠 Interested in AI and developer tooling
- 🇹🇷 Based in Turkey

---

## 🛠️ Technologies

<div align="center">

<img src="https://skillicons.dev/icons?i=ts,js,py,java,react,nextjs,nestjs,nodejs,html,postgres,mysql&theme=dark" />

</div>

<br>

<div align="center">

`TypeScript` · `JavaScript` · `Python` · `Java` · `React` · `Next.js` · `NestJS` · `Node.js` · `HTML` · `PostgreSQL` · `MySQL`

</div>

---

## 🐍 Contribution Activity

<div align="center">

![Snake animation](https://raw.githubusercontent.com/at0mik/at0mik/output/snake.svg)

</div>

---

<div align="center">

### Discord Bot Development

TypeScript · Node.js · Discord.js · Backend Systems · Automation

<br><br>

![Profile Views](https://komarev.com/ghpvc/?username=at0mik&label=PROFILE%20VIEWS&style=flat-square)

<br><br>

**Build. Break. Learn. Repeat.**

</div>
